...
class LowerDenseRule(Rule):
    ...
    def _where(g, x, d, w, b):
        """Accept a data predecessor plus indexed weight and bias inputs."""
        return (
    ...
    def _finalize(g, x, d, w, b, matmul, bias_add, act):
        """Assign OP-IR metadata to preserved and newly created items."""
        dense = g.pmap[d]
        ...
        g.pmap[matmul] = {
            "_type": "MatMul", "name": f"{name}_matmul",
            "shape": shape, "dtype": dtype, "activation": None,
        }
        g.pmap[bias_add] = {
            "_type": "BiasAdd", "name": f"{name}_bias_add",
            "shape": shape, "dtype": dtype, "activation": None,
        }
        g.pmap[act] = {
            "_type": "Activation", "name": f"{name}_activation",
        ...
        for edge, edge_type, index in (
            ((x, matmul), "data", 0),
            ((w, matmul), "parameter", 1),
            ((matmul, bias_add), "data", 0),
            ((b, bias_add), "parameter", 1),
            ((bias_add, act), "data", 0),
        ):
